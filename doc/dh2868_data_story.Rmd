...
When comparing the number of stemmed words per song for each group, it seems that the Beatles have a lower average count (51) than similar rock bands (63).  This could imply that the beatles overal

```{r}

# the number of stemmed words in each song by group
beatles.words_by_song <- sort(sapply(strsplit(beatles_lyrics$stemmedwords, " "), length))
other.words_by_song <- sort(sapply(strsplit(other_lyrics$stemmedwords, " "), length))


# mean, median, and spread
summary(beatles.words_by_song)
summary(other.words_by_song)

# resulting plot
boxplot(beatles.words_by_song, other.words_by_song, horizontal = T, names = c("beatles", "other"))

```

```{r}
other.words_by_song <- other.words_by_song[other.words_by_song < max(other.words_by_song)]
other.words_by_song <- other.words_by_song[other.words_by_song < max(other.words_by_song)]
